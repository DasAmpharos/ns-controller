...
class NsControllerClient:
    def __init__(self, host: str, port: int) -> None:
        ...
        # Ensure buttons are initialized properly
        if not self.current_state.HasField('buttons'):
            self.current_state.buttons.mask = 0

    def _update_button_mask(self, buttons: list[Button], pressed: bool) -> None:
        """Helper to set or clear button bits in the mask."""
        for button in buttons:
            if pressed:
                self.current_state.buttons.mask |= (1 << button)
            else:
                self.current_state.buttons.mask &= ~(1 << button)

    def press(self, buttons: list[Button], send: bool = True, post_delay: float = 0.1) -> None:
        ...
        """
        self._update_button_mask(buttons, True)
        if send:
            self.stub.SetState(self.current_state)
            if post_delay:
                time.sleep(post_delay)

    def release(self, buttons: list[Button], send: bool = True, post_delay: float = 0.1) -> None:
        ...
        Args:
            ...
            send: If True, immediately send the state to the server
        """
        self._update_button_mask(buttons, False)
        if send:
            self.stub.SetState(self.current_state)
            if post_delay:
                time.sleep(post_delay)

    def click(self, buttons: list[Button], down: float = 0.1, post_delay: float = 0.1) -> None:
        ...
        Args:
            ...
            post_delay: Time in seconds to wait after releasing the buttons
        """
        self.press(buttons, send=True, post_delay=0)
        time.sleep(down)
        self.release(buttons, send=True, post_delay=0)
        time.sleep(post_delay)

    def set_stick(self, ls_x: float = 0.0, ls_y: float = 0.0,
                  rs_x: float = 0.0, rs_y: float = 0.0, send: bool = True, post_delay: float | None = None) -> None:
        ...
        Args:
            ...
            send: If True, immediately send the state to the server
            post_delay: Optional delay in seconds after sending the state
        """
        ...
        if send:
            ...
            if not ack or not ack.success:
                logger.warning("Failed to set stick state")
                self.stub.SetState(self.current_state)
        if post_delay:
            time.sleep(post_delay)

    def set_state(self, controller_state: ControllerState, post_delay: float | None = None) -> None:
        """
    ...
    def update_state(self,
        ...
        """
        if buttons_press:
            self._update_button_mask(buttons_press, True)
        if buttons_release:
            self._update_button_mask(buttons_release, False)
        if ls_x is not None:
            self.current_state.ls.x = ls_x
        if ls_y is not None:
            self.current_state.ls.y = ls_y
    ...
    def clear(self, post_delay: float | None = None):
